...
// motion-vector convention the neural runtime expects.
//
// No negation: NVOFA's vectors already point backwards in time, which is the
// convention the runtime wants. This mirrors FlowToMotionPixels in
// MotionVectorMath.h exactly -- see there for why an earlier negation here was
// wrong and how it hid. If one changes, the other must change with it; the
// unit tests guard the C++ side and this shader is written to match it line
// for line.
